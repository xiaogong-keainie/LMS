...
本系统数据库采用关系型模型设计，共包含 **10 个实体（表）**，严格满足课程要求。
设计目标是在 **满足业务完整性的前提下，尽量降低实现复杂度**，便于小组成员并行开发。
...
本文档作为图书管理系统项目的统一需求与设计依据，详细描述了系统功能、模块划分、接口设计以及数据库逻辑模型，可直接用于小组分工与并行开发。系统在保证课程要求完整覆盖的同时，最大限度降低了实现复杂度，具有良好的可执行性与扩展空间。
